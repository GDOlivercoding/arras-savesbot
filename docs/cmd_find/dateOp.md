...
Add integers with a date suffix spaced between each other. Available suffixes:
`y: year, mon: month, d: day, h: hour, min: minute, s: second`

If the year isn't specified it defaults to the current year (ex.: 2025).
If `0 <= year < 100` then `year = year + 2000`.
The tolerance field in the tolerance range never makes any assumptions.
If you're specifying the "runtime" field no assumptions are made as well.
...
- () empty, this will default to the current year in UTC
...
- \>=(2024y 2mon)
- <(2mon)>(2024y 6mon) Note here: the tolerance field will never default its values to anything.
