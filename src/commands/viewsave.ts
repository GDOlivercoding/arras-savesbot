...
const command: Command = {
    ...
    async execute(interaction) {
        const codeID = interaction.options.getString("code-id", true).trimStart().trimEnd();
        const historyIndex = interaction.options.getInteger("history-index");

        let save = saveCollection.getSaveByID(codeID);
        if (!save) {
        ...
        if (target.windowed) {
            ...
            files.push(
                ...
                .setName(`windowed-death-screenshot-${path.name}.${windowed.suffix}`)
                .setDescription(
        ...
        if (target.fullscreen) {
            ...
            files.push(
                new AttachmentBuilder(buf)
                .setName(`fullscreen-death-screenshot-${path.name}.${fullscreen.suffix}`)
                .setDescription(
        ...
        if (target.history) {
            for (let i = 0; i < target.history.length; i++) {
                ...
                output.push(Array(2).fill("`").join(`${i+1}: ${save.path.name}`))
            }
        }
